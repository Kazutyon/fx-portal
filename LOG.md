...
## 2026-09-22 お問い合わせフォームを公開 / Codex

- Googleフォーム「AUXEN お問い合わせ」を作成し、回答者を「リンクを知っている全員」にして公開。
- お名前（任意）、メールアドレス（必須）、お問い合わせ内容（必須）を設定。
- `contact.html` の「フォーム準備中」を公開済みGoogleフォームのiframeへ差し替え。回答者URLの表示確認済み。
- Googleフォームの編集画面ではテーマ色を既定の紫からシアン（`#00bcd4`）へ変更。ただし、公開回答者URLを直接確認すると旧紫色（`#5746e3`）が返っており、公開スナップショットへの反映は未確認。編集画面と公開画面の差分を確認中。
- 公開フォームのiframeを廃止し、AUXEN配色のネイティブ入力UIへ変更。送信先は既存Googleフォームの`formResponse`で、回答保存先は従来どおり。送信後は画面上で完了表示する。
- `contact.html` をCONTACTヒーロー、カテゴリー選択カード、SEND A MESSAGEフォームの2カラム構成へ再設計。SVG線画アイコン、グリッド装飾、シアンの控えめなフォーカス・ホバー、モバイル1カラムを追加。既存サイト共通のヘッダー・フッター・基本色は変更していない。
