...
SUMMARY_P = (
    HERO_P +
    ' なお4Hデイトレ適性ランキングは前日9/16 07:23時点のデータを継続使用しており、本日分の自動更新は本レポート作成時点で反映されていない（要確認）。'
)

TOP_CURRENCY_P = (
    '4Hデイトレ適性ランキング1位（2026-09-16 07:23時点データ・要確認、スコア65・適、ADX26.0でレンジ、直近5日ADRは5年平均の105.1%）。'
...
POINTS_OTHER = [
    ...
    (
        '4Hデイトレ適性ランキングは前日07:23時点データを継続使用（本日分は未取得・要確認）',
        'RemoteTrigger環境からYahoo Financeへのアクセスが制限されているため、本日04:30予定のランキング自動更新ジョブの結果が本レポート作成時点で確認できていない。'
        '前日07:23時点のデータではUSD/JPYが1位（要確認）。'
    ),
]
...
RANKING_ROWS = [
    ('A', 'USD/JPY', 'ランキング1位（スコア65・適）。ADX26.0でレンジ、直近5日ADRは5年平均の105.1%', 'trend-range', '→'),
    ('A', 'EUR/JPY', 'ランキング2位（スコア63・候補）。ADX45.2でレンジ、直近5日ADRは5年平均の84.6%', 'trend-range', '→'),
    ('B', 'NZD/USD', 'ランキング3位（スコア60・候補）。ADX40.3で下降トレンド、直近5日ADRは5年平均の76.3%', 'trend-down', '↓'),
    ('B', 'EUR/USD', 'ランキング4位（スコア58・候補）。ADX46.4で下降トレンド、直近5日ADRは5年平均の66.9%', 'trend-down', '↓'),
    ('B', 'AUD/USD', 'ランキング5位（スコア57・候補）。ADX39.1で下降トレンド、直近5日ADRは5年平均の68.8%', 'trend-down', '↓'),
]

RANKING_NOTE = (
    '※ 4Hデイトレ適性ランキングは2026-09-16 07:23 JST時点のデータ（本日分の自動更新は本レポート作成時点で未反映・要確認）。'
