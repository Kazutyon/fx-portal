...
## BUG-003 GitHub Actions遅延とPages失敗で当日ランキングが反映されない

**発生日:** 2026-07-03
**発見者:** ユーザー
**状態:** 修正済み（次回平日定期実行で継続確認）

### 症状

7月3日の日報は公開されたが、「4H デイトレ適性ランキング」が7月2日のまま更新されなかった。

### 原因

- 06:40 JST予定のランキング生成がGitHub Actions側で07:43 JSTまで遅延し、07:15頃の日報生成より後になった
- 7月3日ランキングJSONのコミットには成功したが、後続のGitHub Pages deployが一時的に失敗し、公開JSONも7月2日のままになった
- ランキング表を日報生成時に静的HTMLへ埋め込んでいたため、後からJSONが更新されても表示へ反映できなかった

### 修正内容

- 定期実行を平日04:30 JSTへ前倒しし、07:01の日報生成まで2時間以上の余裕を確保
- `daytrade-ranking.js` を追加し、トップページ表示時に最新JSONをキャッシュ無効で取得して表と鮮度表示を差し替えるよう変更
- JSON取得失敗時は、日報生成時に埋め込んだ静的ランキングをそのまま表示するフォールバックを維持
- `generate_index.py` と `trigger_prompt.txt` の両テンプレートへ動的読込スクリプトを追加
