...
- FXマーケットニュース枠: `index.html` / `style.css` / `trigger_prompt.txt` に実装済み。GASから自動取得し、2分ごとに自動更新。`もっと見る` は同一ページ内で最大20件まで展開。スマホ表示・自動生成維持ルールも反映。
- 法的ページ完備: about / disclaimer / privacy / terms / contact
- RemoteTrigger: 毎朝7時（JST・平日）自動実行中

## 次の一手

1. ニュース枠の `もっと見る` 展開をcommit/push
2. 公開ページでFXマーケットニュースの自動取得と20件展開が安定しているか確認
3. 次回の自動生成後、ニュース枠・スマホ版トップ・日報スマホ構造が維持されるか確認
...
FXマーケットニュース枠は実装済み。`もっと見る` は外部遷移ではなく、同一ページ内で5件→最大20件へ展開する形に変更中。
次にcommit/pushし、公開ページでニュース自動取得と20件展開を確認する。
