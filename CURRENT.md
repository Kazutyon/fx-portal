...
1. 公開ページでFXマーケットニュースの自動取得と20件展開が安定しているか確認
2. 次回の自動生成後、ニュース枠・スマホ版トップ・日報スマホ構造が維持されるか確認
...
FXマーケットニュース枠はcommit `fbf2743` でpush済み。
`もっと見る` は外部遷移ではなく、同一ページ内で5件→最大20件へ展開する形に変更済み。
公開ページでニュース自動取得と20件展開を確認する。
