...
TODAY      = '2026-09-22'
WEEKDAY    = '火'
HERO_SUB   = '前日9/21(月)はドル円157円台で方向感に乏しい展開。高市首相が指名した日銀審議委員が9/18会合で利上げに反対したことを受け、10月会合での追加利上げのハードルは高いとの見方が広がり、日銀がハト派方向に傾けば円が一段と下落する余地があるとの指摘も出た。ユーロは独仏の政治リスクを意識し軟調、ポンドも一段安の可能性が指摘された。本日9/22(火)は9/21敬老の日と9/23秋分の日に挟まれた「国民の休日」で東京市場が3日連続の休場（シルバーウィーク）となり、KissFXも「注目度の高い米国指標の発表はない」と明記する閑散日。12:10のブロックRBA総裁、17:30・28:30（要確認）のナーゲル独連銀総裁、20:00のラガルドECB総裁、23:05のウィリアムズNY連銀総裁、23:20のジェファーソンFRB副議長など要人発言が中心となる。'
MARKET_HOLIDAY_H3 = '日本市場休場（国民の休日・シルバーウィーク）'
MARKET_HOLIDAY_P  = '9/21（月・敬老の日）と9/23（水・秋分の日）に挟まれた「国民の休日」により、本日9/22（火）も日本市場（東京証券取引所・銀行）が休場（9/21〜23の3連休）。国内勢不在で商いが薄くなりやすい。'
KEY_EVENTS_ITEMS  = [
    '終日 🇯🇵 国民の休日で日本市場休場（シルバーウィーク）',
    '12:10 🇦🇺 ブロックRBA総裁 発言',
    '17:30 🇩🇪 ナーゲル独連銀総裁 発言',
    '20:00 🇪🇺 ラガルドECB総裁 発言',
    '23:05 🇺🇸 ウィリアムズNY連銀総裁 発言',
    '23:20 🇺🇸 ジェファーソンFRB副議長 発言',
]
REPORT_SUMMARY = '日銀ハト派観測でドル円は方向感乏しく、本日も国民の休日で東京休場・薄商い継続'
RISK_LEVEL = 'MEDIUM'
FRB_RATE   = '3.75–4.00%'; FRB_STANCE = 'タカ派（9/16FOMCで0.25%利上げを全会一致で実施、SEPで年内追加利上げ観測）'; FRB_COLOR = 'var(--red)'
BOE_RATE   = '3.75%';      BOE_STANCE = '中立〜ハト派（9/17据え置き・6対3の分裂投票、総合的にハト派的と受け止められポンド安）'; BOE_COLOR = 'var(--muted)'
BOJ_RATE   = '1.25%';      BOJ_STANCE = 'タカ派（9/18会合で0.25%利上げ・反対2、1995年以来の高水準。レートチェック報道で介入警戒も上昇）'; BOJ_COLOR = 'var(--blue)'
ECB_RATE   = '2.50%';      ECB_STANCE = 'タカ派（9/10理事会で0.25%利上げ・全会一致、中東情勢によるインフレ長期化を懸念）'; ECB_COLOR = 'var(--red)'
RBA_RATE   = '4.35%';      RBA_STANCE = 'タカ派寄り中立（8月据え置き・全会一致、追加利上げの可能性は排除せず・次回9/28-29）'; RBA_COLOR = 'var(--red)'
RBNZ_RATE  = '2.75%';      RBNZ_STANCE = 'タカ派（9/2会合で0.25%利上げ、3会合連続の利上げ）'; RBNZ_COLOR = 'var(--red)'
BOC_RATE   = '2.25%';      BOC_STANCE = '中立（9/2会合で7会合連続据え置き・次回10/28）'; BOC_COLOR = 'var(--muted)'
SNB_RATE   = '0.00%';      SNB_STANCE = 'ハト派〜中立（4会合連続据え置き・次回9/25はエコノミスト全員一致で据え置き予想）'; SNB_COLOR = 'var(--muted)'
# ────────────────────────────────────────────────────────
